...
public class GameRoomForMatchParty extends GameRoom {
    private static final Logger logger = getLogger(GameRoomForMatchParty.class);
    ...
    public boolean onCreateRoom(
        ...
        logger.info("GameRoomForMatchParty.onCreateRoom - RoomId : {}, UserId : {}", getId(), gameUser.getUserId());
        try {
            users.put(gameUser.getUserId(), gameUser);
            return true;
        } catch (Exception e) {
            logger.error("GameRoomForMatchParty::onCreateRoom()", e);
            return false;
        }
    }
    ...
    public boolean onJoinRoom(GameUser gameUser, Payload inPayload, Payload outPayload) throws SuspendExecution {
        logger.info("GameRoomForMatchParty.onJoinRoom - RoomId : {}, UserId : {}", getId(), gameUser.getUserId());
        try {
            users.put(gameUser.getUserId(), gameUser);
            logger.info("GameRoomForMatchParty.onJoinRoom - userMatchMaking");
            if (isRefill) {
                if (isRefill) {
                    logger.info("GameRoomForMatchParty.onJoinRoom - roomMatchMaking");
                    ...
                    for (GameUser user : users.values()) {
                        ...
                        logger.info("GameRoomForMatchParty.onJoinRoom - to {} : {}", user.getUserId(), message);
                    }
                ...
                if (users.size() == matchSize) {
                    logger.info("GameRoomForMatchParty.onJoinRoom - userMatchMaking completed");
                    for (GameUser user : users.values()) {
                        ...
                        for (GameUser to : users.values()) {
                            ...
                            logger.info("GameRoomForMatchParty.onJoinRoom - to {} : {}", to.getUserId(), message);
                        }
                    }
                }
            }
            ...
            logger.error("GameRoomForMatchParty::onJoinRoom()", e);
            return false;
        }
    }
    ...
    public boolean onLeaveRoom(GameUser gameUser, Payload inPayload, Payload outPayload) throws SuspendExecution {
        logger.info("GameRoomForMatchParty.onLeaveRoom - RoomId : {}, UserId : {}", getId(), gameUser.getUserId());
        try {
            users.remove(gameUser.getUserId());
            ...
            if (isMadeByUserMatchMaker()) {
                ...
                try {
                    // Refill() 정보 등록
                    logger.info("GameRoomForMatchParty.onLeaveRoom - refill");
                    isRefill = true;
